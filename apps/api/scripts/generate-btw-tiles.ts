...
main().catch((err) => {
  // ДОДАНО (живий інцидент цієї сесії — `BlobStoreSuspendedError: This store has been
  // suspended`, сирий стек-трейс з @vercel/blob без жодної підказки, що робити): це помилка
  // САМОГО Vercel — блоб-сховище заблоковано на РІВНІ акаунта/білінгу/лімітів, а НЕ помилка в
  // цьому скрипті — жодна зміна коду тут це не полагодить, потрібна дія в Vercel dashboard.
  // Розпізнаємо за назвою помилки (§ ЧЕСНО — конкретне ім'я класу `BlobStoreSuspendedError`
  // підтверджено лише за реальним стек-трейсом користувача цієї сесії, не задокументовано явно
  // в .d.ts версії 0.27.1, тому перевіряємо ЯКНАЙШИРШЕ — і за `name`, і про всяк випадок за
  // текстом повідомлення) і додаємо чітку людяну підказку ПОВЕРХ звичайного виводу помилки
  // (не замість — сирий стек лишається для діагностики).
  const isSuspended = err?.name === 'BlobStoreSuspendedError' || /suspended/i.test(String(err?.message ?? ''));
  if (isSuspended) {
    console.error(
      '[generate-btw-tiles] ⚠️ Vercel Blob store ЗАБЛОКОВАНО на стороні Vercel (не баг цього скрипта) — ' +
        'зайдіть у Vercel dashboard → Storage → цей Blob store, і перевірте причину (перевищено ліміт ' +
        'плану/операцій, проблема з білінгом, чи власне рішення Vercel про підозрілу активність) і статус ' +
        'відновлення. Доки стор заблоковано, ЖОДЕН запуск (з --continue чи без) не зможе нічого записати.',
    );
  }
