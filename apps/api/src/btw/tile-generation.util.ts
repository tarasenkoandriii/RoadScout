...
// Перевіряє/скидає кеш комірок, якщо bbox АБО розмір сітки змінився з попереднього запуску —
// довіряти кешу комірок, порахованому для ІНШОЇ географії ЧИ ІНШОГО розбиття на комірки,
// небезпечно (дало б неправильні дані без жодної помітної ознаки браку).
//
...
async function ensureCellCacheValidForBbox(cellCachePrefix: string, bbox: Bbox, cellSizeM: number): Promise<void> {
  ...
  if (existing.length > 0) {
    ...
    try {
      ...
      //
      // ЗВОРОТНА СУМІСНІСТЬ: старий формат файлу (до цього виправлення) містив ГОЛИЙ bbox, без
      // cellSizeM — такий запис ніколи побайтово не збігається з новим `{bbox, cellSizeM}`,
      // тому автоматично трактується як "невалідний" і кеш безпечно скидається один раз при
      // першому запуску після цього деплою (саме той ефект, що й потрібен — стара сітка мала
      // ІНШИЙ розмір комірки, її кеш і так більше не мав значення).
      cachedValidityJson = JSON.stringify(data);
    } catch {
      cachedValidityJson = null;
    }
    if (cachedValidityJson === validityJson) return; // bbox і розмір сітки не змінились — кеш валідний
  }

  // bbox/розмір сітки не збігається (або кеш ще порожній, або читання не вдалось) — краще
  // безпечно почати "з нуля", ніж ризикнути змішати комірки різної географії/розбиття без
  // жодної помітної ознаки браку.
