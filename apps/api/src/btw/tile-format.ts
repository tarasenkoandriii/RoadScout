...
//
// ⚠️ ЦЕЙ файл — окрема копія від apps/btw/lib/tile-format.ts (клієнтський порт для Worker'а,
// § вже задокументований у цій сесії принцип "дві копії — обидві мають синхронно оновлюватись",
// раніше застосований до Overpass-клієнта й геометричного движка). Під час першого впровадження
// поля `name` цю копію було помилково пропущено (відредаговано лише клієнтську) — саме тому
// `generate-btw-tiles.ts` падав з TS2353 на реальному запуску користувача. Тепер обидві копії
// синхронізовано.
