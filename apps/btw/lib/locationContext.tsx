...
interface LocationContextValue {
  ...
  // ще раз.
  //
  // ВИПРАВЛЕНО (аудит, знайдено після видалення фази 'intro' з app/scan/page.tsx — § коментар
  // там) — раніше тут стверджувалось, що кнопка "Начать сканирование" на /scan викликає
  // ЦЕЙ САМЕ `refresh()` як fallback. Це застаріло: тієї кнопки більше немає (запит дозволів
  // тепер стартує автоматично на монтуванні `/scan`), і `requestPermissions()` там ніколи не
  // викликав `refresh()` контексту — він завжди мав власний, повністю окремий шлях визначення
  // позиції (dev-override -> geolocation), незалежний від цього провайдера. `refresh()` наразі
  // не викликається НІДЕ за межами самого провайдера — лишається як публічне API на майбутнє
  // (напр. кнопка "Повторить" на головному екрані, якщо колись знадобиться), не мертвий код,
  // але й не той fallback, що описувався тут раніше.
  refresh: () => void;
}
